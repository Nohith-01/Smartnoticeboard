...
const marqueeData = async (req,res) =>{
  ...
  const resp = marquee.create({first,second});
...
const postData = async (req, res) => {
  const { category, text, secure_url, fileType } = req.body;
  if (category === 'sports') {
    sportsData.create({ category, text, dispUrl: secure_url, fileType });
  } else if (category === 'circulars') {
    circulars.create({ category, text, dispUrl: secure_url, fileType });
  } else if (category === 'placements') {
    placements.create({ category, text, dispUrl: secure_url, fileType });
  } else if (category === 'events') {
    events.create({ category, text, dispUrl: secure_url, fileType });
  } else if (category === 'others') {
    others.create({ category, text, dispUrl: secure_url, fileType });
  }
  else{
    dataModel.create({category, text, dispUrl: secure_url, fileType});
  }
};
